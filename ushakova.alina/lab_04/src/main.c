...
void sort_int(int argc, char *argv[]) {
	...
	int failed = mergesort(array_int, argc - 2, sizeof(int), int_gt_comparator);
	if (failed == 1) {
...
void sort_str(int argc, char *argv[]) {
	char **array_str = malloc((argc - 2) * sizeof(argv[2]));
	for (int i = 0; i < argc - 2; i++) {
		array_str[i] = argv[i + 2];
	}
	int failed = mergesort(array_str, argc - 2, sizeof(array_str[0]), str_gt_comparator);
	if (failed == 1) {
		printf("Error: memory allocation failed\n");
		free(array_str);
		return;
...
void sort_char(int argc, char *argv[]) {
	...
	int failed = mergesort(array_char, argc - 2, sizeof(char), char_gt_comparator);
	if (failed == 1) {
		free(array_char);
		printf("Error: memory allocation failed\n");
		return;
